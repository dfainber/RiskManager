...
description: Análise de concentração e diversificação de risco do fundo multiestratégia Galapagos Evolution FIC FIM CP. Calcula três camadas de métricas que não existiam antes — (1) utilização histórica de VaR por estratégia (MACRO, SIST, FRONTIER, CREDITO, EVO_STRAT, CAIXA) em percentis próprios; (2) diversification benefit, ou seja, quanto a correlação entre estratégias está reduzindo o VaR agregado vs. a soma linear; (3) correlação realizada rolling entre os PnLs das estratégias, para identificar quando elas se alinham direcionalmente. O foco é detectar o cenário perigoso de "bull market alignment" — quando MACRO + SIST + FRONTIER ficam simultaneamente long e a diversificação some. Use sempre que o usuário pedir "como está o Evolution", "concentração de risco do Evolution", "diversificação das estratégias", "alinhamento direcional", "percentil do VaR", "risk on em todas as estratégias", "o Evolution está muito para um lado", "correlação entre MACRO e SIST", "briefing do Evolution", ou qualquer pergunta sobre a composição agregada de risco do fundo multiestratégia.
---
