...
| `market_states_fetch.py`| Read-only loader do pickle externo `Markets states/regime_panel/artifacts_glpg.pkl` (fail-soft em stale/missing) |
| `market_states_renderers.py`| Strip colorido no Resumo + sub-tab "Estados" no Mercado (Risk State 5-bucket + Quadrant Q1-Q4 por Trade/Trend/Tail) |
...
| `LOTE45`    | `LOTE_BOOK_STRESS`                          | **Stress oficial replicável** — `SUM(MACRO+SPECIFIC)` no LEVEL canonical por fundo (TREE='Main'). Mapping: MACRO_Q/ALBATROZ/EVO em L=1; QUANT/IDKA3Y/IDKA10Y em L=2; MACRO/BALTRA quebram (engine bug) — ver `project_rule_lote_book_stress_level_per_fund.md`. **Implementação pendente** (kit ainda usa `LOTE_FUND_STRESS_RPM.MACRO_STRESS` isolado em `fetch_risk_history`). |
...
8. **VaR DoD exposure NAV-axis (§2.13b — IDKA cota timing)** → **PARKED 2026-05-02 (late+2)**. Tentativa de shift bench por 2 bdays revertida — premissa do review (SHARE[D] reflete D-2 axis) é frágil: "cotização D-2" no Brasil é regra de transação (subscrição/resgate liquidam D+2), NÃO de cálculo de NAV diário. Engine popula `LOTE_TRADING_DESKS_NAV_SHARE.SHARE[D]` com preços de mercado de D, igual ao bench → axis-D nas duas séries, sem mismatch. Bench correto é **same-day**; se faltar bench em D, dropar a observação (não shiftar). Solução real: helper calculation que valide axis empiricamente (comparar SHARE[D] de IDKA com revaluation independente em D antes de qualquer correção). Função `compute_idka_bvar_hs` continua unwired em produção; revert deixa código no estado pré-fix.
...
11. **Aesthetic carryovers ainda abertos**: ~~§3.6~~ → **DONE 2026-05-02 (late+2)** — per-fund footnote (10× → 1 exec full + 5 tooltips compactos no per-fund cards que mencionam "dado/tomado", omitido no resto). `<div title="...">` tooltip native HTML, sem nova CSS. §3.8 (meeting-port hardcoded hex sweep + extend `_BG_HEX_MAP`) ainda pendente.
...
14. **Stress replication v2** (descoberto 2026-05-02 stress session) — fórmula real é `SUM(MACRO_STRESS) + SUM(SPECIFIC_STRESS) FROM LOTE_BOOK_STRESS WHERE LEVEL = canonical_level_per_fund AND TREE='Main'`. Replica 5/7 fundos com erro <0.06% NAV (QUANT, MACRO_Q, ALBATROZ, IDKA 3Y, IDKA 10Y; EVOLUTION 0.34%). MACRO/BALTRA quebram (engine bug em L=1 truncation + multi-tree). **Pendente**: implementar `fetch_fund_stress_v2` em `data_fetch.py` substituindo `fetch_risk_history` (que usa MACRO_STRESS isolado de `LOTE_FUND_STRESS_RPM` — gap sistemático -1.3pp QUANT, +0.3-0.7pp MACRO/EVO). Mapping LEVEL em `project_rule_lote_book_stress_level_per_fund.md`. Scripts em `scripts/investigate_quant_stress*.py`. Atualizar `mandato-QUANT.json._calibracao_stress` (obsoleto).
16. ~~**Market States integration**~~ → **DONE 2026-05-02 (late+3)**. Read-only consumer of external project pickle (`F:\Bloomberg\Quant\MODELOS\DFF\Markets states\regime_panel\artifacts_glpg.pkl`). Renderiza no Resumo (strip colorido com Risk State + Master Quadrant + per-duration pills) e no Mercado em nova sub-tab "Estados" (5-segment Risk State gauge + RoR sparkline + 2×2 Quadrant grid). 5 cores Risk State (Medo Extremo→Ganância Extrema) + 4 cores Quadrant (Q1-Q4). Fail-soft: pickle ausente/stale (>7d) → seção oculta, sem crash. `MARKET_STATES_ARTIFACTS_PATH` + `MARKET_STATES_MAX_AGE_DAYS` env-overridable. Módulos: `market_states_fetch.py`, `market_states_renderers.py`. Wired em `_build_market_section` + `_build_summary_view`. Memory: `project_market_states_integration.md`. Pendente baixa-prioridade: extender `_BG_HEX_MAP`/`_TEXT_COLOR_MAP` em `generate_risk_report_meeting.py` para os 9 hex novos (light-theme port).
15. ~~**VaR replication**~~ → **CLOSED 2026-05-02 (validated, no fix needed)**. User entregou screenshots APM 4/28/2026 (QUANT, MACRO_Q headline panels + MACRO, EVOLUTION primitive dumps). Kit replica EXATO em QUANT (-0.42%) e MACRO_Q (-0.18%). Em MACRO (-0.57% kit vs -0.64% GUI) e EVO (-0.29% kit vs -0.34% GUI) o kit reporta MENOR — mas é porque kit usa look-through diversificado (correto) e GUI nas screenshots estava em modo NÃO-diversificado (cotas opacas). Per user: "the GUI can be without diversification" — confirmou que -0.57%/-0.29% são os valores corretos com diversificação. Sem mudança de código. Pattern: funds sem cotas (QUANT puro equity, MACRO_Q puro ETF/futures) match exato; funds com cotas (MACRO holds Macro Access; EVO holds Macro/Quant SubAs + Frontier + FIDCs) → kit menor pelo benefício de diversificação. Probe scripts em `scripts/investigate_var_replication{,2,3}.py` (paralelos a `investigate_quant_stress*.py`). Memory: `project_rule_var_kit_is_lookthrough.md`, `project_session_2026_05_02_var_replication.md`. Adjacents parqueados (low priority): 4 horizontes (P/H × 1d/5d) disponíveis em `LOTE_FUND_STRESS` para MACRO_Q (kit só surface P.VaR1); RPM só tem 1d (sem 5d para QUANT/MACRO/EVO/BALTRA — exigiria √5 scaling ou source alternativo).
