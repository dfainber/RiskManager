...
8. ~~**VaR DoD exposure NAV-axis (§2.13b — IDKA cota timing)**~~ → **DONE 2026-05-02 (late+2)**. `compute_idka_bvar_hs` agora aceita `bench_lag_bdays: int = 2` e shifta `r_bench` por N rows do calendário inner-joined antes de calcular `r_fund - r_bench`. Default 2 alinha com cotização IDKA D-2; pass 0 pra fundos same-day. Função stub não-wired em produção, fix preventivo pra próximo consumer não herdar overstatement 10-30%. Tests verdes (26 metrics).
