...
from pa_renderers import _pa_render_name, _pa_filter_alpha
...
from section_assembly import build_secondary_sections
...
def _build_agg_rows(rf_expo_maps: dict | None, df_frontier, df_quant_sn,
    ...
    if df_frontier is not None and not df_frontier.empty:
        ...
        if not stocks.empty and fr_nav:
            brls = stocks["% Cash"].astype(float).to_numpy() * fr_nav
            keep = abs(brls) >= 1_000
            for prod, brl in zip(stocks.loc[keep, "PRODUCT"].tolist(), brls[keep]):
                agg_rows.append({
                    "fund": "Frontier", "factor": "Equity BR",
                    "product": prod, "brl": float(brl), "unit": "BRL",
                })
...
def _render_master_html(
    *,
    summary_html: str, quality_html: str, market_section_html: str,
    pnl_section_html: str, peers_section_html: str, sections_html: str,
    alerts_html: str, mode_tabs_html: str, fund_subtabs_html: str,
    report_subtabs_html: str,
    vardod_modal_html: str, vardod_data_script: str,
    pmovers_modal_html: str, pmovers_data_script: str,
    navigation_js: str, cards_js: str,
) -> str:
    """Compose the final standalone HTML document. CSS / JS blobs come
    from module-level imports (MAIN_CSS, VARDOD_CSS, PMOVERS_CSS, etc.);
    only per-page dynamic strings are passed in."""
...
def build_html(d: ReportData) -> str:
    ...
    # All sections beyond the per-fund Risk Monitor card — exposure, distribution,
    # vol regime, single-names, PA, ALBATROZ + BALTRA + QUANT + EVOLUTION exposure,
    # credit look-through, RF maps, ALBATROZ risk budget, FRONTIER LO, Análise.
    _extra_secs, pmovers_data_script, pmovers_modal_html = build_secondary_sections(
        d, td_by_short=td_by_short, series_map=series_map,
    )
    sections.extend(_extra_secs)
    ...
    navigation_js = main_navigation_js(
        fund_reports_js=fund_reports_js, report_labels_js=report_labels_js,
        DATA_STR=DATA_STR, fund_shorts_js=fund_shorts_js,
        fund_labels_js=fund_labels_js, fund_order_js=fund_order_js,
    )
    cards_js = cards_data_js(
        _book_pnl_json=_book_pnl_json, _peers_json=_peers_json,
        _peers_eopm_json=_peers_eopm_json,
    )
    html = _render_master_html(
        summary_html=summary_html, quality_html=quality_html,
        market_section_html=market_section_html,
        pnl_section_html=pnl_section_html, peers_section_html=peers_section_html,
        sections_html=sections_html, alerts_html=alerts_html,
        mode_tabs_html=mode_tabs_html, fund_subtabs_html=fund_subtabs_html,
        report_subtabs_html=report_subtabs_html,
        vardod_modal_html=vardod_modal_html, vardod_data_script=vardod_data_script,
        pmovers_modal_html=pmovers_modal_html, pmovers_data_script=pmovers_data_script,
        navigation_js=navigation_js, cards_js=cards_js,
    )
    return html
